...
public class AccountSQL {
	...
	public int selectBalance2(int admin_no, Connection conn) throws Exception {
		int balance = 0;
		String sql = "SELECT BALANCE FROM K_ADMIN WHERE ADMIN_NO = ?";
		PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	public int updateBalance2(int admin_no, int price, int balance, Connection conn) throws Exception {
		String sql = "UPDATE K_ADMIN SET BALANCE = ? WHERE ADMIN_NO = ?";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, price + balance);
		pstmt.setInt(2, admin_no);
		return pstmt.executeUpdate();
	}
	...
	public int insertAccount2(int user_no, int admin_no, int price, Connection conn) throws Exception {
		String sql = "INSERT INTO ADMIN_ACCOUNT(USE_NO, ADMIN_NO, USER_NO, PRICE) VALUES(SEQ_USER_USE_NO.NEXTVAL, ?, ?, ?)";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, admin_no);
		pstmt.setInt(2, user_no);
		pstmt.setInt(3, price);
